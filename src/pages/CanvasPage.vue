...
const flowNodes = computed(() => store.nodes.map(node => ({ ...node, position: { ...node.position }, data: { ...node.data }, label: node.data.title })))
...
<template>
  <section class="workspace" aria-label="Workflow canvas">
    ...
    <VueFlow v-else :nodes="flowNodes" :edges="flowEdges" :min-zoom="0.2" :max-zoom="2"
